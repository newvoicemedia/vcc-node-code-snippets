...
const argv = yargs
  .usage('$0 [-f filePath]', 'Starts process of deleting interactions and its content with GUIDs provided in file')
  .options({
    'filePath': {
  ...
  .check((argv) => {
    ...
    throw new Error("Please provide 'filePath' option")
  })
  .version(false)
  .help()
  .alias('help', 'h')
...
const icsClient = new IcsClient(process.env.CLIENT_ID, process.env.CLIENT_SECRET, process.env.REGION);
...
(async () => {
  ...
  try {
    ...
    while (guids.length > 0) {
      ...
      const failures = await icsClient.deleteInteractions(guids).then(r => r.failures);
      if (failures.length > 0) {
        console.warn(`Deletion of some GUIDs failed, check ${failuresFile} file for more details`);
